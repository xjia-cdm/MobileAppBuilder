...
class ModelBuilder { 
  ...
  def methodMissing(ModelNode parentNode, String name, args) { 
    ...
    if (ui || transition || state || action || graphics) { 
	  ...
	  errors << new ErrorMessage(file: xj.mobile.Main.scriptFile,
								 line: curLine,
    ...
    if (closure) { 
      ...
      try { 
		...
		if (isListEntityClass(e.type.name)) { 
		  ...
		  errors << new ErrorMessage(file: xj.mobile.Main.scriptFile, line: line, message: e.message)
		} else { 
		  throw e
		}
      }
  ...
  def handleParameter(owner, key, value) { 
	...
	if (verbose) info "[ModelBuilder] handleParameter(): Parameter: key=${key} pvalue=${pvalue} class=${pvalue?.class?.name}"
	if (pvalue != null) {
	  value = pvalue
	} else if (value instanceof Map) { 
	  if (verbose) info "[ModelBuilder] handleParameter(): value instanceof Map"
	  value.each { k, v -> 
	  ...
	  if (verbose) info "[ModelBuilder] handleParameter(): value instanceof List"

...
import static xj.mobile.common.ViewUtils.getActionName
...
"""
    ...
    if (delegateAction || widgetType == 'Text') { 
      if (delegateAction && (actionCode || widgetType == 'Text')) { 
		...
		if (textFieldUpdateCode) { 
		  ...
		  textFieldUpdateCode = actionCode
		}
      }
    } else { 
      if (actionCode) { 		
  ...
  void handleKeyboardAction() {
	String code = ''
	if (textFieldUpdateCode) { 
	  code = '\n' + indent(textFieldUpdateCode)
	}
	methodScrap += """
- (void)dismissKeyboard {
    [self.view endEditing:YES];${code}
}
"""
  }

  ...
  def handleDelegates() { 
    if (delegateActions) { 
      delegateActions.each { name, delegate -> 
		info "[IOSClass.handleDelegates] ${name} ${delegate.widgetType} ${delegate.delegateName}";
		...
		if (delegate.actions) { 
          if (delegate.actions.size() == 1) { 
			...
			delegate.actions.each { action -> 
			  info "[IOSClass.handleDelegates] param=${param} var=${action.name} code=${action.code}";
			  if (action.code) { 
				if (actionCode == null) { 
				  actionCode = ''
				} else { 

...
"""
  ...
  def getListItemMembers() { 
	...
	if (hasData) { 
	  def type = listItemType['data']
	  if (listItemDataVarType) { 		
		switch (listItemDataVarType) { 
		case STRING_TYPE: type = 'String'; break; 
		case MAP_TYPE: type = 'Bundle'; break; 
		case OBJECT_TYPE: type = 'Bundle'; break; 
  ...
  String initListItem(item) { 
	...
	if (hasData) {
	  ...
	  if (item.next instanceof Map && 
		  item.next.data != null) { 
		if (item.next.data instanceof Map) { 
		  classModel.auxiliaryClasses << 'Utils'
		  def kvlist = []
		  item.next.data.each { k, v -> kvlist << k << v}
		  data = 'Utils.makeBundle(' + kvlist.collect{ "\"${it}\"" }.join(', ') + ')'
		} else { 
		  data = "\"${item.next.data}\""
		}
	  }

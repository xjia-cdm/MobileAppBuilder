...
class Lang { 
  ...
  static buildMainPage(sections) { 
    def pageTitle = 'MADL API Documentation'
	...
	mainBuilder.html { 
	  ...
	  ndbody(framed) { 
		div(id: 'Content') { 
		  ndtopic('MADL Objects', 'main', 1, null) //{ mkp.yieldUnescaped(' ') }
					...
					if (vdef.type == vtype) { 
					  tr { 
						...
						td(class: 'CDLDescription') { 
						  ...
						  mkp.yield(vdef.platforms.collect { "UI.${it}" })
						}
					  }
					}
				  }
  ...
  static generateDefPage(title, name, sections, attrMap) { 
    ...
    defBuilder.html { 
	  ndhead("MADL API Documentation -- ${title}: ${name}", null)
	  ndbody(framed) { 
  ...
  static widgetContent(builder, title, name, attrMap) { 
	builder.div(id: 'Content') { 
	  ...
	  if (entry.platforms.contains(Language.Platform.Neutral) &&
		  WidgetMap._widgets[name]) { 
		[ 'iOS', 'Android' ].each { pname ->
		  def p = pname.toLowerCase()
		  def nativeNames = ''
		  def pwname = WidgetMap._widgets[name][p]

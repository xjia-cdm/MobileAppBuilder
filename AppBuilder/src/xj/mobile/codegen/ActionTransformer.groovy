...
class ActionTransformer extends ClassCodeExpressionTransformer { 
  ...
  void setContext(ViewProcessor viewProcessor, ModelNode widget, Parameter[] params = null) {  
    ...
    this.widget = widget
    this.params = params
	this.platform = viewProcessor.platform
  }
      ...
      if (widget &&
		...
		if (viewProcessor.generator.attributeHandler.apiResolver.hasPropertyDef(widget.nodeType, exp.name)) { 
		  if (transformResults && needTransformGetViewProperty(widget.id, exp.name)) { 
			...
			gvpExp.platformViewType = widget.getPlatformWidgetName(platform)
			...
			if (vp && objExp.name in vp.widgetTable.keySet()) { 
			  if (needTransformSetViewProperty(objExp.name, propExp.text)) { 
				...
				if (w) { 
				  ...
				  svpExp.platformViewType = w.getPlatformWidgetName(platform)
			  ...
			  if (Language.isWidget(tname)) { 
				...
				svpExp.platformViewType = WidgetMap.getPlatformNameForWidgetType(tname, platform)
				if (embedded) svpExp.owner = 'owner'
		...
		if (vp && objExp.name in vp.widgetTable.keySet()) { 
		  ...
		  if (needTransformGetViewProperty(objExp.name, propExp.text)) { 
			...
			if (w) { 
			  ...
			  gvpExp.platformViewType = w.getPlatformWidgetName(platform)
		  ...
		  if (Language.isWidget(tname)) { 
			...
			gvpExp.platformViewType = WidgetMap.getPlatformNameForWidgetType(tname, platform)
			if (embedded) gvpExp.owner = 'owner'
		  ...
		  if (isDummy) { 
			...
			return gdpExp 
		  } else if (viewProcessor.generator.attributeHandler.apiResolver.hasPropertyDef(widget.nodeType, objExp.name)) { 
			if (widget && needTransformGetViewProperty(widget.id, objExp.name)) { 
